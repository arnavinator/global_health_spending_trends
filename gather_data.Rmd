...
# import new dataset on population and population density per country. Remove
# irrelevant columns and use population to convert number of men/women in each
# row to instead find the proportion of male/female for each country, creating a
# standardized quantity of comparision regardless of population

pop_dens <- read_csv("raw-data/pop+pop_density.csv", skip = 1) %>% 
  clean_names() %>% 
  mutate(country = x2) %>% 
  mutate(pop_type = series) %>% 
  select(country, year, pop_type, value) %>% 
  filter(pop_type %in% c("Population mid-year estimates (millions)",
  ...
  pivot_wider(names_from = pop_type, values_from = value) %>% 
  clean_names() %>% 
  mutate(prop_male = population_mid_year_estimates_for_males_millions/population_mid_year_estimates_millions) %>% 
  mutate(prop_female = population_mid_year_estimates_for_females_millions/population_mid_year_estimates_millions) %>% 
  mutate(prop_014 = population_aged_0_to_14_years_old_percentage/100) %>% 
  mutate(pop_millions = population_mid_year_estimates_millions) %>% 
  mutate(pop_density = population_density) %>% 
  select(country, year, pop_millions, prop_male, prop_female, prop_014, pop_density)
...
# total health spending by income and reigion
...
# graphs of the average health spending per person for each reigion, showing the
# trends of each income group compared to the next higher income group to show how rate always increases
...
```{r testing3, echo = FALSE}
# is the steeper increase in higher income countries correlated to greater oop/private spending?
...
# maybe look at mediteranean specifically cuz less DAH
...
```{r testing4, echo = FALSE}


# There is a logarithmic relationship between health spending and income... we see scary realization that greater healthcare spending leads to greater deaths?
options(scipen=999)
cor1 <- spend_death %>%  
  filter(age == "All Ages") %>% 
  filter(sex == "Both") %>% 
...
# this shows diminishing returns for healthcare. I hypothesize that as income group increases, the effectiveness of healthcare decreases due to communicable vs noncommunicable disease
...
#this shows global epidemiological transition !
...
# we can also look at the impact of age and gender effects on the total health 
# spending per capita 

# for reference, here is total health spending per capita for all ages and gender
spend_death %>% 
  filter(age == "All Ages") %>% 
  filter(sex == "Both") %>% 
  ggplot(aes(x = the_per_cap_mean, y = val)) +
...
# younger age groups seem to have greater sensitivity with to increase in
# healthcare spending than older groups. However, whether their association is
# due to greater public appeal of children or children weaker or a mix of both,
# or not causal at all, is difficult to predict
spend_death %>% 
  filter(sex == "Both") %>% 
...
# looking at sensitivity based on gender, no difference in effectiveness. This
# suggests that factors of public do not affect the community, rather it is the
# simple effectiveness of more spending
